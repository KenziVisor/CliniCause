...
class RunContext:
    ...
    dataset_pkl_path: Path | None
    causal_stage_selector: str
    selected_causal_stages: list[str]
    stages: dict[str, StageRecord]
    summary_path: Path


def normalize_causal_stage_selector(selector: str) -> tuple[str, list[str]]:
    raw_selector = str(selector).strip().lower()
    if raw_selector == "all":
        return "all", list(STAGE_SEQUENCE)
    parts = [part.strip() for part in raw_selector.split(",") if part.strip()]
    valid_order = ",".join(STAGE_SEQUENCE)
    if not parts:
        raise argparse.ArgumentTypeError(
            f"causal stage selection must be non-empty; valid order: {valid_order}"
        )
    if len(parts) != len(set(parts)):
        raise argparse.ArgumentTypeError(
            f"causal stages cannot be duplicated; valid order: {valid_order}"
        )
    unknown = [part for part in parts if part not in STAGE_SEQUENCE]
    if unknown:
        raise argparse.ArgumentTypeError(
            f"unknown causal stages {unknown}; valid order: {valid_order}"
        )
    if parts != STAGE_SEQUENCE[: len(parts)]:
        raise argparse.ArgumentTypeError(
            "causal stages must be a non-empty ordered prefix; "
            f"valid order: {valid_order}"
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="all",
        help=(
            "Causal stage scope: 'all' or a comma-separated ordered prefix of "
            + ",".join(STAGE_SEQUENCE)
        ),
    )
    parser.add_argument(
    ...
    try:
        (
            args.causal_stage_selector,
            args.selected_causal_stages,
        ) = normalize_causal_stage_selector(args.causal_stages)
...
def build_run_context(args: argparse.Namespace) -> RunContext:
    ...
    downstream_selected = bool(
        set(args.selected_causal_stages)
        & set(STAGE_SEQUENCE[2:])
    )
    ...
    if downstream_selected:
        ...
        mortality_device = "not-selected"
        mortality_device_source = "causal-stage-scope"
    ...
    if args.latent_tags_dir is None:
        if args.validate_config_only or "majority_vote" not in args.selected_causal_stages:
            latent_tags_dir = Path(".").resolve()
        else:
            raise ValueError("Provide --latent-tags-dir when majority_vote is selected.")
    else:
        latent_tags_dir = Path(args.latent_tags_dir).expanduser().resolve()

    dataset_pkl_value = args.dataset_pkl_path
    if dataset_pkl_value is None and args.validate_config_only and downstream_selected:
        dataset_pkl_value = "dataset.pkl"
    if dataset_pkl_value is None and downstream_selected:
        raise ValueError(
            "Provide --dataset-pkl-path when downstream causal stages are selected."
        )
    ...
    return RunContext(
        ...
        dataset_pkl_path=(
            Path(dataset_pkl_value).expanduser().resolve()
            if dataset_pkl_value is not None
            else None
        ),
        causal_stage_selector=args.causal_stage_selector,
        selected_causal_stages=list(args.selected_causal_stages),
        stages=stages,
...
def validate_config_only(args: argparse.Namespace, context: RunContext) -> None:
    config_contract = (
        "main.py"
        if set(context.selected_causal_stages) & set(STAGE_SEQUENCE[2:])
        else context.stages["graph"].script_path
    )
    resolved = validate_script_config(config_contract, context.config)
    print_resolved_config_summary(config_contract, context.config, resolved)
    ...
    if args.dataset_pkl_path is not None and context.dataset_pkl_path is not None:
        resolve_file_path(str(context.dataset_pkl_path), "dataset pickle")

    print(f"  causal_stage_selector: {context.causal_stage_selector}")
    print(f"  selected_causal_stages: {','.join(context.selected_causal_stages)}")
    ...
    for stage_name, builder in [
        ...
        if stage_name not in context.selected_causal_stages:
            print(
                f"    {stage_name}: SKIPPED - Not selected by causal stage scope: "
                f"{context.causal_stage_selector}."
            )
            continue
...
def prepare_output_directories(context: RunContext) -> None:
    ...
    for stage_name in context.selected_causal_stages:
        ensure_directory(context.stage_dirs[stage_name])


def validate_context(context: RunContext) -> None:
    if "majority_vote" in context.selected_causal_stages:
        resolve_directory_path(str(context.latent_tags_dir), "latent-tags dir")
    if set(context.selected_causal_stages) & set(STAGE_SEQUENCE[2:]):
        if context.dataset_pkl_path is None:
            raise ValueError("A dataset pickle is required by selected downstream stages.")
        resolve_file_path(str(context.dataset_pkl_path), "dataset pickle")
...
def write_run_summary(
    ...
    if context.causal_stage_selector != "all":
        terminal_stage = context.stages[context.selected_causal_stages[-1]]
        terminal_artifact = primary_output_path(terminal_stage)
    summary_payload = {
        ...
        "dataset_pkl_path": (
            str(context.dataset_pkl_path)
            if context.dataset_pkl_path is not None
            else None
        ),
        "causal_stage_selector": context.causal_stage_selector,
        "selected_causal_stages": context.selected_causal_stages,
...
def orchestrate(context: RunContext) -> tuple[bool, str | None]:
    ...
    if context.selected_causal_stages != STAGE_SEQUENCE:
        skip_reason = (
            "Not selected by causal stage scope: "
            f"{context.causal_stage_selector}."
        )
        mark_stages_skipped(
            context,
        ...
        for index, stage_name in enumerate(context.selected_causal_stages, start=1):
            stage = context.stages[stage_name]
            if is_causalpfn_model(context) and stage_name in {
                "analyze_cate_results",
                "permutations_test",
            }:
                mark_stage_skipped(stage, lock, CAUSALPFN_SKIP_REASON)
                continue
            reset_stage_directory(context.stage_dirs[stage_name])
            print(
                f"[{index}/{len(context.selected_causal_stages)}] "
                f"Running selected causal stage: {stage_name}"
            )
            if not run_stage_subprocess(
                stage,
                builders[stage_name](context),
                context.repo_root,
                env,
                lock,
            ):
                overall_error = stage.error
                mark_stages_skipped(
                    context,
                    context.selected_causal_stages[index:],
                    lock,
                    f"Skipped because the selected {stage_name} stage failed.",
                )
