...
## P8 artifact and CausalPFN-ingestion build update

P8 replaced the Figure 1 placeholder with a deterministic vector PDF, retained the checked Figure 2 source/predicate, retained the checked Table 1/Table 2 values, created a separate anonymous technical appendix, and added release-neutral package preparation content. The P8 audit is `evidence/P8_figure_table_value_checks.md`.

The final CausalPFN NeurIPS 2025 citation key is `balazadeh2025causalpfn`. It is used only for method-level description; local numerical claims remain mapped to checked result CSVs. The PDF was normalized into the literature corpus, cataloged, checksummed, and recorded in `logs/causalpfn_literature_ingestion_report.md`.

The main build command succeeded with the AAAI Author Kit. `paper.pdf` has 8 US-Letter pages; technical content ends on page 7 and references occupy page 8. The Abstract remains empty. Fonts are embedded and no Type 3 font occurs. Figure 1 and Figure 2 are vector PDFs with embedded fonts; Figure 1 and Figure 2 each regenerated byte-identically on consecutive runs. A P9 scientific compression allowance remains necessary before writing the Abstract; no global compression was performed in P8.

---

## P8 Figure 1 narrow-repair build update
