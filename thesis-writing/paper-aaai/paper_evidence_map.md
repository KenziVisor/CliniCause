...
| C19 | CausalPFN supplies a complementary third estimator perspective | Contribution/method | Intro/Causal Estimation | Checked results plus `balazadeh2025causalpfn` | External method evidence plus local empirical evidence | Medium | The citation supports bounded method framing; P5 supports local numerical results and diagnostic limits remain separate | Fig. 2 | None | SUPPORTED WITH QUALIFICATION |
...
| C50 | CausalPFN is a meaningful complementary estimator evaluated on the same prespecified pairs to test whether directions are DML-specific | Sec. 4.2, paragraph 2 | Checked CATE experiment matrix; `results_decision_register.md`; `balazadeh2025causalpfn` | SUPPORTED WITH QUALIFICATION | Citation supports the bounded method-level description; local archive still determines diagnostic coverage and results |
...
| C76 | CausalPFN's broad concordance makes it a prominent complementary estimator result | Sec. 6.2 | C64; `checked_cate_candidates.csv`; P5 numerical audit | SUPPORTED WITH QUALIFICATION | External citation now supports method framing; local evidence and diagnostic limits remain distinct |
...
| C103 | CausalPFN is empirically evaluated as a complementary estimator with bounded method-level Related Work treatment | Sec. 2, paragraph 3 | `balazadeh2025causalpfn` for method framing; C50, C64--C65, C76 for local role/results | SUPPORTED WITH QUALIFICATION | Citation does not validate local effects, proxy definitions, identification, or diagnostic parity |
...
|---|---|---|---|---|---|---|
| G-EVD-01 | RESOLVED | `balazadeh2025causalpfn`, catalog row, checksum, and normalized local PDF now provide the verified CausalPFN primary citation | P8 literature-ingestion record | Supports bounded method attribution | Local identification, diagnostics, source/version, and release gaps remain separate | No |
...
## P8 artifact and citation update

- Figure 1 is the deterministic vector artifact `figures/figure1_pipeline.pdf`, generated without data by `figures/generate_figure1_pipeline.py`. It explicitly separates design-time LLM proposals from runtime deterministic source and patient-level estimation.
- Figure 2 remains source-exact: `checked_cate_candidates.csv`, original sampling, the three estimator-specific main-text statuses, 19 dataset--exposure combinations, and 57 values. Its P8 audit is in `evidence/P8_figure_table_value_checks.md`.
- Table 1 and all 32 Table 2 values retain their checked P5 mappings. The complete 19x3 original estimator matrix, 57 original/downsampled comparisons, 19 matching rows, and diagnostic coverage are generated by `supplement/generate_supplement_tables.py` from checked aggregate CSVs only.
- `G-EVD-01` is resolved: `balazadeh2025causalpfn` cites the verified NeurIPS 2025 proceedings record; local PDF `literature/papers/causal_causalpfn_balazadeh_et_al_2025.pdf`; SHA-256 `cd3dcd6017745e31db617503f77fa31f6bac454b3be5b3748c4e51d8f5ab3950`. It supports bounded CausalPFN method descriptions in Related Work and Evaluation. P5 remains the authority for local 18/19 evidence; diagnostic and identification limitations remain open.
