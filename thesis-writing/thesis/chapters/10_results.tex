...
The three author-selected main-result PNGs were not inserted.  The frozen figure-selection register classifies both original-cohort ranking images as \texttt{BLOCKED\_VALUE\_CONFLICT} because their displayed labels implement a different estimator aggregation from the image description, and it classifies the direction-count image the same way because it combines original and outcome-downsampled analyses.  Presenting those images as support for the checked tables would conflict with the frozen packet.
...
CausalPFN CATE execution completed for all 19 original-cohort comparisons.  The three estimators agreed in mean-effect direction for 18 of the 19 dataset--exposure comparisons: MIMIC was concordant for 9 of 9 and PhysioNet for 9 of 10 (Table~\ref{tab:results-pfn-comparison}).  The sole exception was PhysioNet \nolinkurl{LAT_SHOCK}: CausalForestDML was negative ($-0.014$), LinearDML was negative ($-0.027$), and CausalPFN was slightly positive (0.0041).
