...
\caption[Archived predictive test performance.]{Archived predictive test performance for the four learned models.  AUROC, AUPRC, and minRP are macro-averaged across non-degenerate rule-derived proxy-label targets on the archived test split.}
...
\caption[MIMIC-III CausalForestDML results.]{Primary original-cohort MIMIC-III CausalForestDML results ($n=26{,}845$).  Prevalence and mean model-estimated CATE are proportions on their archived scales.}
...
\caption[MIMIC-III CausalForestDML exposure ranking.]{Original-cohort MIMIC-III mean model-estimated CATE values from CausalForestDML.  All nine prespecified proxy-state exposures are shown.  The ordering is descriptive and does not establish statistical significance, clinical importance, or population-average causal effects.}
...
\setlength{\tabcolsep}{4pt}
\begin{longtable}{@{}L{0.34\textwidth}rrlL{0.16\textwidth}@{}}
\caption[PhysioNet CausalForestDML results.]{Primary original-cohort PhysioNet CausalForestDML results ($n=7{,}993$).  Prevalence and mean model-estimated CATE are proportions on their archived scales.}
...
\caption[Original-cohort matching and support summaries.]{Original-cohort descriptive matching and empirical-support summaries.  ``Difference'' is the descriptive matched-pair outcome difference; it is not a model-estimated CATE.}
...
\caption[Original-cohort Forest and Linear comparison.]{Original-cohort comparison of mean model-estimated CATE from CausalForestDML and LinearDML.}
...
\setlength{\tabcolsep}{4pt}
\begin{longtable}{@{}lL{0.43\textwidth}rrc@{}}
\caption[CausalPFN results and DML direction agreement.]{Original-cohort exploratory CausalPFN mean model-estimated CATE and direction agreement with both DML estimators.}
...
\begin{table}[htbp]
\centering
\caption[Within-resource original-cohort estimator stability.]{Within-resource estimator stability for the original cohorts.  Rank correlations and RMSE values are ranges across the three estimator pairs.  RMSE is disagreement on the model-estimated mean-CATE scale, not estimator accuracy.}
...
Cardiac injury & 18.8--25.9 / 11.2--12.2 & 17.0 unadjusted \cite{babuin2008troponin} & Broadly compatible in scale; populations and injury definitions differ. \\
Inflammation / sepsis & 14.9--16.1 / 6.7--7.2 & 5.0 matched \cite{shankarhari2018sepsis} & Lower than both CliniCause ranges; constructs, populations, and horizons differ. \\
...
Coagulation / hematologic & --- / 0.4--2.5 & 19.5 matched \cite{stephan1999thrombocytopenia} & Discrepant; severe thrombocytopenia is narrower, and MIMIC has no separate proxy. \\
Respiratory & 3.6--5.0 / 6.3--6.5 & 15.0, targeted minimum loss-based estimation \cite{torres2021ards}; other horizon & Discrepant; ARDS definitions and horizons are not equivalent to the proxy. \\
...
\caption[Robustness and diagnostic availability.]{Robustness and diagnostic availability.  Counts are exposure-level statuses, not effect estimates.}
