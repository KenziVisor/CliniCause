...
MIMIC-III is a critical-care electronic health-record database used here to instantiate the second, separate ICU resource \cite{johnson2016mimiciii}.  The implemented workflow follows a clinical time-series benchmark style in that it transforms raw ICU events into stay-level time-series examples suitable for prediction, while adapting the output contract for this thesis pipeline \cite{harutyunyan_2019_mimiciii_benchmark}.  The resulting analysis-ready representation is constructed from source observations rather than being a synthetic patient or outcome generator.  The repository does not establish a tracked raw-data snapshot, extraction date, or final processed artifact hash.
...
\begin{table}[htbp]
\centering
\caption[Processed artifact contracts.]{Processed artifact contracts used by the causal and STraTS repositories.}
