...
\begin{longtable}{@{}p{0.10\textwidth}p{0.12\textwidth}p{0.09\textwidth}p{0.07\textwidth}p{0.11\textwidth}p{0.12\textwidth}p{0.10\textwidth}p{0.10\textwidth}@{}}
...
\midrule
\endhead
PhysioNet 2012 and MIMIC-III & CausalForest\allowbreak DML & Original cohort & Dataset-specific project graph & Majority-vote binary proxy-state columns & Matching, saved-CATE analysis, and permutations & Archived run family with stage records & Numbered producing config path recorded; local file missing \\
...
PhysioNet 2012 and MIMIC-III & CausalForest\allowbreak DML, LinearDML, CausalPFN & Outcome-downsampled cohort & Dataset-specific project graph & Majority-vote binary proxy-state columns & Same family-specific checks, interpreted for a different empirical population & Archived run families with stage records & Numbered producing config path recorded; downsampling setting needs manifest confirmation \\
\bottomrule
\end{longtable}
\endgroup
...
Predictive splits and checkpoint/export mapping & Training summaries, exports, wrappers & Split IDs and seeds, checkpoint hashes, producing commands, and copy record & Final prediction provenance incomplete & Per-export lineage \\
