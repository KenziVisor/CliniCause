...
\begin{figure}[p]
\centering
\includegraphics[width=0.98\linewidth,height=0.78\textwidth,keepaspectratio]{physionet_causal_dag.png}
\caption[Project-specified PhysioNet 2012 DAG.]{Project-specified PhysioNet 2012 DAG encoded by the active causal-pipeline graph source; arrows are assumed, not learned or clinically validated.}
\label{fig:physionet-causal-dag}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.98\linewidth,height=0.78\textwidth,keepaspectratio]{mimic_causal_dag.png}
\caption[Project-specified MIMIC-III DAG.]{Project-specified MIMIC-III DAG encoded by the active causal-pipeline graph source; arrows are assumed, not learned or clinically validated.}
...
\caption[Causal assumptions and interpretation boundaries.]{Causal assumptions required for interpreting the estimator outputs.}
...
For heterogeneous-effect modeling, the pipeline includes two EconML DML estimators and an exploratory CausalPFN branch.  CausalForestDML is primary, LinearDML the secondary comparator, and CausalPFN exploratory.  Double machine learning estimates nuisance outcome and treatment models before estimating residualized effects, reducing first-order sensitivity to nuisance-model error under appropriate assumptions \cite{chernozhukov2018dml,oprescu_et_al_2019_econml}.  LinearDML uses random-forest nuisance models with a linear final effect model; CausalForestDML uses the same nuisance family and a causal-forest final stage \cite{wager2018causalforest}.  CausalPFN is a prior-fitted transformer for amortized in-context causal-effect estimation \cite{balazadeh2025causalpfn}.  Its exact historical producing package version and checkpoint are unresolved, and it lacks DML-equivalent uncertainty, sensitivity, and permutation support.  ICU observational-design guidance remains relevant to all three estimators \cite{smit_2023_causal_inference_icu_scoping_review}.
...
\caption[Implemented causal estimators and interpretation limits.]{Implemented causal estimators and interpretation limits.}
