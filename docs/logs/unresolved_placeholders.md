...
| file | chapter and section | exact placeholder | reason | evidence needed | expected resolution stage | status |
| --- | --- | --- | --- | --- | --- | --- |
| `thesis-writing/thesis/chapters/06_predictive_modeling.tex` | Chapter 6, C6.1 Self-Supervised STraTS Pretraining | `[VALIDATION REQUIRED: recover a predictive manifest linking each final STraTS-family supervised checkpoint, pretraining checkpoint, export command, and exported CSV]` | Archived supervised and pretraining checkpoints can now be hashed, and export logs record argument sets, but final exported CSVs are not independently mapped to the intended pretraining and supervised checkpoints. | Predictive manifest with explicit checkpoint-to-export links, producing command record, source code state, processed artifact hashes, and archive-copy records. | Predictive result-manifest preparation before final Chapter 6/10 result claims | open |
...
| `thesis-writing/thesis/chapters/06_predictive_modeling.tex` | Chapter 6, C6.3 Prediction Export and Normalization | `[VALIDATION REQUIRED: record the final export split, source processed-pickle hash, split-generation seed or ID manifest, source checkpoint, and archive-copy provenance for each prediction CSV]` | Archived logs record `predict_split=all` and `seed=2023`, and checkpoint files are hashable, but split identifiers, processed-pickle hashes, checkpoint-to-export proof, full shell commands, and archive-copy provenance remain incomplete. | Per-CSV manifest covering split IDs, source processed pickle hash, explicit checkpoint-to-export link, producing command, source version, and archive copy path. | Predictive result-manifest preparation before Chapter 10 | open |
...
| `thesis-writing/thesis/chapters/07_causal_methodology.tex` | Chapter 7, C7.1 Dataset-Specific DAGs | `[VALIDATION REQUIRED: recover the exact graph source commit, config, command, and output hashes for the canonical DAG artifacts]` | The copied DAG PNGs and pickles are internally consistent. Archived run summaries now provide exact graph-stage argv arrays, CWDs, return codes, timestamps, and output paths, but source commits, numbered configs, historical input/output hashes, and archive-copy direction remain missing. | Graph script commit, numbered config CSVs, historical input/output hash manifest, and archive-copy record for each canonical graph artifact. | Result-manifest preparation before final methods freeze | open |
