...
import pickle
import numpy as np
from split_contract import validate_split_contract
...
# Generate split.
train_valid_ids = sorted(oc.loc[oc.subset!='a'].ts_id.tolist())
np.random.seed(123)
...
validated = validate_split_contract(
    ts['ts_id'].tolist(), oc['ts_id'].tolist(),
    train_ids, valid_ids, test_ids,
)
train_ids = list(validated.train)
valid_ids = list(validated.val)
