...
from split_contract import require_ids_present, validate_split_contract
import os
import re
...
class Dataset:
    def __init__(self, args) -> None:
        ...
        full_train_ids = np.asarray(validated_splits.train, dtype=object)
        full_val_ids = np.asarray(validated_splits.val, dtype=object)
        full_test_ids = np.asarray(validated_splits.test, dtype=object)

        try:
            run_parts = args.run.split('o')
            if len(run_parts) != 2:
                raise ValueError
            run, totalruns = list(map(int, run_parts))
        except (AttributeError, TypeError, ValueError) as exc:
            raise ValueError(f"Invalid model run specification {args.run!r}.") from exc
        if totalruns < 1 or run < 1 or run > totalruns:
            raise ValueError(f"Invalid model run specification {args.run!r}.")
        if not (0 < args.train_frac <= 1):
            raise ValueError(f"train_frac must be in (0, 1], got {args.train_frac!r}.")

        num_train = int(np.ceil(args.train_frac*len(full_train_ids)))
        start = int(np.linspace(0,len(full_train_ids)-num_train,totalruns)[run-1])
        train_ids = full_train_ids[start:start+num_train]
        num_val = int(np.ceil(args.train_frac*len(full_val_ids)))
        start = int(np.linspace(0,len(full_val_ids)-num_val,totalruns)[run-1])
        val_ids = full_val_ids[start:start+num_val]
        ...
        full_cohort_ids = np.concatenate((full_train_ids, full_val_ids, full_test_ids))
        require_ids_present(
            full_cohort_ids, curr_ids,
            'Filtering variables to those observed in the selected training split',
        )
        args.logger.write('# train, val, test TS: '+str([len(train_ids), len(val_ids), len(test_ids)]))
        sup_ts_ids = full_cohort_ids
        ...
        # Get y and N
        latent_df = pd.read_csv(args.latent_csv_path, dtype=str)
        ...
        if latent_df['ts_id'].isna().any():
            raise ValueError('latent CSV contains missing canonical stay identifiers.')
        ...
        expected_id_set = set(sup_ts_ids_str)
        latent_id_set = set(latent_df['ts_id'])
        missing_ids = expected_id_set - latent_id_set
        extra_ids = latent_id_set - expected_id_set
        if missing_ids or extra_ids:
            raise ValueError(
                "latent CSV cohort does not exactly match the processed canonical cohort: "
                f"missing={len(missing_ids)} {_format_id_sample(sorted(missing_ids))}; "
                f"extra={len(extra_ids)} {_format_id_sample(sorted(extra_ids))}"
            )
        ...
        self.authoritative_splits = {
            'train': list(full_train_ids),
            'val': list(full_val_ids),
            'test': list(full_test_ids),
        }
        self.metadata_cohort_ids = self.sup_ts_ids
        ...
        if args.finetune:
            pt_var_path = args.preprocessing_state_path
            if not pt_var_path:
                raise ValueError('Fine-tune architecture requires preprocessing state.')
