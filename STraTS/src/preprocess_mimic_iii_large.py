...
def validate_processed_artifact(events, oc, train_ids, valid_ids, test_ids):
    ...
    events, oc = restrict_frames_to_split_cohort(
        events, oc, train_ids, valid_ids, test_ids,
    )


    validated = validate_split_contract(
        events['ts_id'].tolist(), oc['ts_id'].tolist(),
        train_ids, valid_ids, test_ids,
    )
    train_ids = np.asarray(validated.train, dtype=object)
    valid_ids = np.asarray(validated.val, dtype=object)
    test_ids = np.asarray(validated.test, dtype=object)
...
all_sup_subjects = np.sort(icu.SUBJECT_ID.unique())
np.random.seed(0)
